...
func edgeFile(t *testing.T, content string) cnfg.Parser {
	t.Helper()

	return cnfg.Decode(json.Unmarshal, writeFile(t, "edges.json", content))
}

func TestNilPointerStructIsFilled(t *testing.T) {
	cfg, err := cnfg.Parse(Edges{}, flags("-limits-max-conns", "7"))
	if err != nil {
...
func TestPointerTextUnmarshaler(t *testing.T) {
	cfg, err := cnfg.Parse(Edges{}, flags("-when", "2026-09-15T00:00:00Z"))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestSliceElementError(t *testing.T) {
	_, err := cnfg.Parse(Edges{}, env("NUMS=1,x"))
...
func TestUnexportedAndUnderscoreNames(t *testing.T) {
	...
	ptr := 3
	_, _ = cnfg.Parse(Edges{When: &when, Ptr: &ptr, Bytes: []byte("hi")}, cnfg.FlagSet(set, []string{"-h"}))
...
func TestFlagsFromArgs(t *testing.T) {
	...
	cfg, err := cnfg.Parse(Edges{}, cnfg.Flags())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestOptionalPassesTheRestThrough(t *testing.T) {
	...
	cfg, err := cnfg.Parse(Edges{}, cnfg.Optional(cnfg.Decode(json.Unmarshal, file)))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if _, err := cnfg.Parse(Edges{}, cnfg.Optional(cnfg.Decode(json.Unmarshal, broken))); !errors.Is(err, cnfg.ErrDecodeFile) {
...
func TestDecodeFlagNotStruct(t *testing.T) {
	...
	_, err := cnfg.Parse(42, cnfg.DecodeFlag(json.Unmarshal, set, "config", nil))
