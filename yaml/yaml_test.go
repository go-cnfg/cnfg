...
func TestFile(t *testing.T) {
	cfg, err := cnfg.Parse(defaults(), yaml.File[Config](writeFile(t, "config.yaml", "addr: \":1111\"\ntimeout: 90s\nserver:\n  tls-cert: file.pem\n")))
	if err != nil {
...
func TestFileFlag(t *testing.T) {
	path := writeFile(t, "config.yaml", "addr: \":1111\"\ntimeout: 90s\nserver:\n  tls-cert: file.pem\n")
	...
	cfg, err := cnfg.Parse(defaults(),
		yaml.FileFlag[Config](set, "config", args),
...
func TestOptional(t *testing.T) {
	...
	if _, err := cnfg.Parse(defaults(), cnfg.Optional(yaml.File[Config](missing))); err != nil {
		t.Errorf("optional file should be ignored: %v", err)
	}
	if _, err := cnfg.Parse(defaults(), yaml.File[Config](missing)); !errors.Is(err, cnfg.ErrReadFile) {
		t.Errorf("got %v, want ErrReadFile", err)
	}
}

func TestBrokenFile(t *testing.T) {
	_, err := cnfg.Parse(defaults(), yaml.File[Config](writeFile(t, "config.yaml", "addr: [\n")))
...
func TestDir(t *testing.T) {
	...
	cfg, err := cnfg.Parse(defaults(), yaml.Dir[Config](dir))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestDirMissing(t *testing.T) {
	cfg, err := cnfg.Parse(defaults(), yaml.Dir[Config](filepath.Join(t.TempDir(), "conf.d")))
	if err != nil {
